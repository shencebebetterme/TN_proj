...
int num_states = 40;// final number of dots in the momentum diagram
...
//normalize TM s.t its first element is 1
//prevent TM from being too small
void showElements(const ITensor& tm){
    Index Ti = tm.index(1);
    Index Tj = tm.index(2);
    double ele = 0;
    for (int i=1; i<10; i++){
        for (int j=1; j<10; j++){
            ele = elt(tm,Ti=i,Tj=j);
            //ele = 0;
            printf("\nThe (%d,%d) element of TM is %f\n", i, j, ele);
        }
    }
}

//todo: when len_chain>15, on entry to DGEMM parameter number 3 had an illegal value
...
//////////////////////////////////////////////////////////////////////////////
    ...
    //std::vector<ITensor> As = {};

    for (int i : range1(len_chain-1)){
        ...
        //M /= TrA;
        //Print(M);
        printf("eating %dth A tensor\n",i);
        TM *= delta(previous_ri, li);
        printf("%dth delta tensor eaten\n",i);
        TM *= Ai;
        //TM *= (Ai * delta(previous_ri, li));
        printf("%dth A tensor eaten\n\n",i);
        //normalizeTM(TM);
        //TM /= TrA;
    ...
    Index r_last = addTags(r,"site="+str(len_chain-1));
